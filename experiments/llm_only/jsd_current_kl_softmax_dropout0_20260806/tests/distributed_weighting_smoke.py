...
def run(temperature: float, rank: int, world_size: int) -> None:
    ...
    if rank == 0:
        ...
        if not math.isclose(actual, expected, rel_tol=1e-6, abs_tol=1e-6):
            raise AssertionError(f"T={temperature}: DDP scaling mismatch {actual} vs {expected}")
        print(f"PASS T={temperature}: DDP gradient={actual:.8f}")
...
def main() -> None:
    ...
    try:
        for temperature in (0.05, 0.1):
            run(temperature, rank, world_size)
    finally:

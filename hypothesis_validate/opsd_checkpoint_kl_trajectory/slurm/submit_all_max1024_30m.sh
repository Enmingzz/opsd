...
SBATCH=${ROOT}/slurm/run_checkpoint_max1024_30m.sbatch
MANIFEST=${ROOT}/outputs/slurm_jobs_metric_noocr_max1024_20260729.tsv
CHECKPOINT_ROOT=/scratch/enmingzz/outputs/llm_only/checkpoints/llm_only_random_decontam_v1_20260713/opsd
...
submit() {
  ...
  local adapter="$3"
  local job_id
  job_id=$(sbatch --parsable --job-name="kl1024-${label}" "${SBATCH}" "${label}" "${adapter}")
  printf '%s\t%s\t%s\t%s\t%s\n' \

#!/bin/bash
# Run inside an existing 4xL40S allocation:
#   srun --jobid=<ALLOC_JOB_ID> --overlap --ntasks=1 --cpus-per-task=8 \
#     bash opsd/slurm/run_mme_eval_step1000_r025_in_alloc.sh
set -euo pipefail
...
run_shard() {
  ...
  local start="$3"
  local limit="$4"
  local out="${OUT_DIR}/shards/eval_step1000_r025_mme_shard${shard}.jsonl"
  local log="${OUT_DIR}/shards/eval_step1000_r025_mme_shard${shard}.log"
  echo "Starting MME shard ${shard}: start=${start} limit=${limit} gpu=${gpu} at $(date)"
  CUDA_VISIBLE_DEVICES="${gpu}" python opsd/scripts/eval_qwen25vl_pruned_student.py \
    ...
    > "${log}" 2>&1
  echo "Finished MME shard ${shard} at $(date)"
}
...
cat \
  ...
  "${OUT_DIR}/shards/eval_step1000_r025_mme_shard2.jsonl" \
...
echo "MME full eval complete at $(date)"

...
RUN_GROUP=${RUN_GROUP:-aokvqa_first4_reasoning_$(date +%Y%m%d_%H%M%S)}
MASTER_PORT_BASE=${MASTER_PORT_BASE:-29821}
...
printf '%s\n' "${RUN_GROUP}" > "${OUT_ROOT}/latest_aokvqa_first4_reasoning_train_run.txt"

{
  ...
  printf '04\tfreeze_sft_teacher_opsd_nogt_reasoning_gbs8\t%s/aokvqa_opsd_sft_teacher_freeze_nogt_gbs8.yaml\treasoning\t%s\n' "${GENERATED_CONFIG_DIR}" "${MAX_NEW_TOKENS}"
} > "${LOGDIR}/manifest.tsv"

...
MASTER_PORT_BASE=${MASTER_PORT_BASE:-29751}
...
STAGE3_LOG_JSON="${STAGE3_OUT}/training_log.jsonl"
STAGE3_RESUME="${STAGE3_OUT}/step_16000"
if [ ! -d "${STAGE3_RESUME}" ]; then
  echo "missing resume checkpoint: ${STAGE3_RESUME}" >&2
  exit 1
fi
if [ -f "${STAGE3_LOG_JSON}" ]; then
  BACKUP="${STAGE3_LOG_JSON}.before_resume_$(date +%Y%m%d_%H%M%S).bak"
  cp "${STAGE3_LOG_JSON}" "${BACKUP}"
  python - "${STAGE3_LOG_JSON}" <<'PY'
import json
import sys
...
with path.open(encoding='utf-8') as f:
    for line in f:
        if not line.strip():
            continue
        obj = json.loads(line)
        if int(obj.get('step', -1)) <= 16000:
            rows.append(line)
path.write_text(''.join(rows), encoding='utf-8')
print(f'truncated {path} to {len(rows)} rows <= step 16000')
PY
fi

echo "[$(date --iso-8601=seconds)] resume 03_ema_opsd_nogt_official_gbs8 from step_16000" | tee -a "${LOGDIR}/sequence.log"
torchrun \
  --nproc-per-node=4 \
...
FREEZE_CONFIG="${GENERATED_CONFIG_DIR}/aokvqa_opsd_sft_teacher_freeze_gt_gbs8.resume.yaml"
python - "configs/visionzip_aokvqa/aokvqa_opsd_sft_teacher_freeze_gt_gbs8.yaml" "${FREEZE_CONFIG}" "${BASE_OUT}/01_sft_official_gbs8/final" <<'PY'
import sys
from pathlib import Path
import yaml
src = Path(sys.argv[1])
...
echo "[$(date --iso-8601=seconds)] start 04_freeze_sft_teacher_opsd_gt_official_gbs8" | tee -a "${LOGDIR}/sequence.log"
torchrun \
  --nproc-per-node=4 \
  --master-port="$((MASTER_PORT_BASE + 1))" \
  visionzip_aokvqa/train.py \
  --config "${FREEZE_CONFIG}" \
  --output_dir "${BASE_OUT}/04_freeze_sft_teacher_opsd_gt_official_gbs8" \
  2>&1 | tee "${LOGDIR}/04_freeze_sft_teacher_opsd_gt_official_gbs8.log"
echo "[$(date --iso-8601=seconds)] done 04_freeze_sft_teacher_opsd_gt_official_gbs8" | tee -a "${LOGDIR}/sequence.log"
